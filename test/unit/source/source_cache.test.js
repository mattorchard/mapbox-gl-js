    ...
    t.test('regular tiles', (t) => {
        ...
        eventedParent.on('data', (e) => {
            if (e.sourceDataType === 'metadata') {
                ...
                t.equal(tiles[0].bufferedGeometrySpace, 'tile');
                t.deepEqual(round(tiles[0].bufferedBounds), {min: {x: 4080, y: 4034}, max: {x:8192, y: 8162}});

                t.equal(tiles[1].tile.tileID.key, 528);
                t.equal(tiles[1].tile.tileSize, 512);
                t.equal(tiles[1].bufferedGeometrySpace, 'tile');
                t.deepEqual(round(tiles[1].bufferedBounds), {min: {x: 0, y: 4034}, max: {x: 4112, y: 8162}});
    ...
    t.test('reparsed overscaled tiles', (t) => {
        ...
        eventedParent.on('data', (e) => {
            if (e.sourceDataType === 'metadata') {
                ...
                t.equal(tiles[0].bufferedGeometrySpace, 'tile');
                t.deepEqual(round(tiles[0].bufferedBounds), {min: {x: 4088, y: 4042}, max: {x:8192, y: 8154}});

                t.equal(tiles[1].tile.tileID.key, 529);
                t.equal(tiles[1].tile.tileSize, 1024);
                t.equal(tiles[1].bufferedGeometrySpace, 'tile');
                t.deepEqual(round(tiles[1].bufferedBounds), {min: {x: 0, y: 4042}, max: {x: 4104, y: 8154}});
